  ...
  <div class="modal-card" style="height:500px">
      ...
      <section class="modal-card-body">
        ...
        <b-field
            ...
            :message="formSupplier.status_kedatangan.message"
            :type="formSupplier.status_kedatangan.type">
            <b-autocomplete
                v-model="cariPelanggan"
                icon="magnify"
                placeholder="Status Kedatangan"
                :value="supplier.status_kedatangan"
                :open-on-focus="true"
                :data="filteredDataSupplier"
                @select="option => {supplierTemp.status_kedatangan = option.status_kedatangan; supplierTemp.id_supplier = option.id_supplier}"
                @input="clearError(formSupplier.status_kedatangan)"
      ...
      cekSupplier() {
        ...
        if (this.supplierTemp.id_supplier == 0 ||
            this.supplierTemp.nama_supplier == '') {
              count++
              this.formSupplier.nama_supplier.message = 'Member belum terpilih dengan benar'
